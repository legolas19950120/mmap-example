...
int main(int argc, char **argv)
{
    ...
    if ((vq.desc = (unsigned long long*)mmap(NULL, vq.num*sizeof(unsigned long long), PROT_READ |   
                PROT_WRITE, MAP_SHARED | MAP_POPULATE, fd, 0)) == (void *)-1) {  //映射共享内存，本来mmap返回值是void *，强制转换我们自己要的类型
        perror("mmap");  
    }  
